...
public class StatisticsCounterRx {
    ...
    public static class Result {
        ...
        public Pair<Buyer, Long> getBestBuyer() {
            Pair<
                    Buyer, Long> result = new Pair<>(null, -1L);
            for (ConcurrentMap.Entry<
                    Buyer, Long> entry : this.totalSpendPerBuyer.entrySet())
        ...
        public Pair<
                Product, Integer> getMostPopularProduct() {
            Pair<
                    Product, Integer> result = new Pair<>(null, -1);
            for (Map.Entry<
                    Product, Integer> entry : this.totalSellCountPerProduct.entrySet())
                if (entry.getValue() > result.getValue1())
                    result = new Pair<>(entry.getKey(), entry.getValue());
            return result;
        }
        ...
        public Pair<
                Product.Category, Integer> getMostPopularCategory() {
            Pair<
                    Product.Category, Integer> result = new Pair<>(null, -1);
            for (Map.Entry<
                    Product.Category, Integer> entry : this.totalSellCountPerCategory.entrySet())
                if (entry.getValue() > result.getValue1())
                    result = new Pair<>(entry.getKey(), entry.getValue());
            return result;
        }
    ...
    public static class NonConcurrentResult {
        Map<
                Buyer, Long> totalSpendPerBuyer;
        Map<
                Product, Integer> totalSellCountPerProduct;
        Map<
                Product.Category, Integer> totalSellCountPerCategory;
        long totalProfit;
        ...
        public NonConcurrentResult(Map<
                Buyer, Long> totalSpendPerBuyer,
    ...
    static class CallableLongAction implements Callable<Integer> {
        ...
        public CallableLongAction(
                Purchase purchase, Result result) {
        ...
        public Integer call() {
            ...
            for (
                    Product product : purchase.getProducts().keySet()) {
    ...
    private static Observable<Integer> processPurchase(
            Purchase purchase, Result res) {
        return Observable.fromCallable(new CallableLongAction(purchase, res));
    }

    public static Observable<Integer> processPurchases(List<
            Purchase> purchases, Result res) {
        return Observable.
                fromIterable(purchases).
                flatMap(p -> processPurchase(p, res).subscribeOn(Schedulers.io()));
    }

    public static Result countRxWithCallableAction(List<
            Purchase> purchases) {
    ...
    public static Result countRxSimple(List<
            Purchase> purchases) {
        Result result = new Result();
        Observable.
                fromIterable(purchases).
    ...
    static class CallableLongActionNonConcurrent implements Callable<Integer> {
        ...
        public CallableLongActionNonConcurrent(
                Purchase purchase, NonConcurrentResult result) {
            this.purchase = purchase;
            this.result = result;
        }
    ...
    private static Observable<Integer> processNonConcurrentPurchase(
            Purchase purchase, NonConcurrentResult res) {
        return Observable.fromCallable(new CallableLongActionNonConcurrent(purchase, res));
    }

    public static Observable<Integer> processNonConcurrentPurchases(List<
            Purchase> purchases, NonConcurrentResult res) {
        return Observable.
                fromIterable(purchases).
                flatMap(p -> processNonConcurrentPurchase(p, res).subscribeOn(Schedulers.io()));
    }

    public static NonConcurrentResult countNonConcurrentRxWithCallable(List<
            Purchase> purchases) {
        Map<
                Buyer, Long> totalSpendPerBuyer = new ConcurrentHashMap<>();
        Map<
                Product, Integer> totalSellCountPerProduct = new ConcurrentHashMap<>();
        Map<
                Product.Category, Integer> totalSellCountPerCategory = new ConcurrentHashMap<>();
    ...
    public static Result countRxNestedObservablesWithConcurrentMap(List<
            Purchase> purchases) {
...
//                                        System.out.println(Thread.currentThread().getName());
    ...
    public static NonConcurrentResult countRxNestedObservableWithMap(List<
            Purchase> purchases) {
...
//                                        System.out.println(Thread.currentThread().getName());
    ...
    public static Result countStandardCollectorsParallel(List<
            Purchase> purchases) {

        Collector<
                Purchase, ?, ConcurrentMap<
                Buyer, Long>>
        ...
        Collector<
                Purchase, ?, Long> totalProfitCollector =
    ...
    public static NonConcurrentResult countStandardCollectors(List<
            Purchase> purchases) {

        Collector<
                Purchase, ?, Map<
                Buyer, Long>> totalSpendPerBuyerCollector =
                Collectors.toMap(
        ...
        Collector<
                Purchase, ?, Map<
                Product, Integer>> totalSellCountPerProductCollector = Collectors.flatMapping(
                purchase -> purchase.getProducts().entrySet().stream(),
                Collectors.toMap(
                        Map.Entry::getKey,
                        Map.Entry::getValue,
        ...
        Map<
                Buyer, Long> totalSpendPerBuyer = purchases.stream().collect(totalSpendPerBuyerCollector);
        Map<
                Product, Integer> totalSellCountPerProduct = purchases.stream().collect(totalSellCountPerProductCollector);
        Map<
                Product.Category, Integer> totalSellCountPerCategory =
    ...
    public static Result countRxWithParallelCollectors(List<
            Purchase> purchases) {
    ...
    public static Result countRxWithCollectors(List<
            Purchase> purchases) {
        var result = new Result();
        Observable.fromIterable(purchases).
                flatMap(
    ...
    public static NonConcurrentResult countRxGroupByCategory(List<
            Purchase> purchases) {
        ...
        Observable.fromIterable(purchases).
                ...
                flatMapSingle(
                        (GroupedObservable<
                                Product.Category, Pair<Map.Entry<
                                Product, Integer>,
                                Buyer>> groupObservable) ->
                                groupObservable.
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        int n = 10;
        List<
                Purchase> purchases = Purchase.PurchaseGenerator.generatePurchases(n, delay,
